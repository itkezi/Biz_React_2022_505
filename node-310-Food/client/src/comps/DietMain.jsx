...
node 서버로부터 데이터를 fetch(SELECT ALL) 하여 state 변수에 담는 코드가 작성될 곳
*/
...
List 보여주고 다음에 tem을 클릭하거나 변경하는 코드를 추가하려면
어디에서 state 데이터를 만들어야 할 것인가를 잘 고민해봐야 한다                
DietMain에서 DBfetch하고 item 등을 변화시키는 event도 선언하여 
Main Component가 감싸고 있는 child Component들이 공유할 수 있도록 설계 한다
*/
...
const DietMain = () => {
  ...
  // state 변수 값이 변화 될 때마다 useEffect 함수 실행된다
  // state 변수의 변화를 감지하여 어떤 event를 실행할 때 사용하는 방법

  // 두번째 매개변수에 빈(blank) 배열([])을 설정하면
  // 컴포넌트가 rendering될 때 한번만 실행하도록 하는 방법
  ...
  */
  const removeFoodItem = (d_id) => {
    fetch(`/food/remove/${d_id}`, { method: "DELETE" })
      .then((res) => res.text())
      .then((result) => {
        const fillterBody = foods.filter((food) => food.d_id !== d_id);
        setFoods(fillterBody);
      });
  };

  /*
  child component에 전달해야할 데이터와 함수가 많아지는 경우
  개별적으로 전달하는 것 보다 하나의 객체로 묶어서 전달하기
  */
  const params = {
    foods,
    removeFoodItem,
