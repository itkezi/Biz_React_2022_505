...
이때 return 내의 컴포넌트 tag의 가장 바깥을 감싸서 한 개의 tag로 묶어야 한다
 */
const BucketMain = () => {
  /* 
  ...
  새로운 배열과 기존의 배열을 교환하는 방식으로 이뤄진다
  */
  ...
  const onKeyDown = (e) => {
    if (e.keyCode === 13) {
      const bucket = {
        ...
        b_start_date: moment().format("YY[-]MM[-]DD / HH:mm"), // .format("YYYY[-]MM[-]DD HH:mm:ss")
  ...
  return (
    <>
      <div>
        <input
          onKeyDown={onKeyDown}
          placeholder="귀여운 양에게 시킬 일을 입력하세요"
